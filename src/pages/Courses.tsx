...
const Courses = () => {
  ...
  const filtered = useMemo(() => {
    return courses.filter((c) => {
      ...
      const matchSearch = (c.title?.toLowerCase() ?? "").includes(search.toLowerCase()) || catName.toLowerCase().includes(search.toLowerCase());
      const matchCat = !selectedCategory || catName === selectedCategory;
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="gradient-bg pt-32 pb-16 lg:pb-20">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <h1 className="text-3xl lg:text-5xl font-bold text-primary-foreground mb-4">Nossos Cursos</h1>
          <p className="text-primary-foreground/80 max-w-xl mx-auto text-lg">Explore nossa coleção de cursos e encontre o caminho ideal para sua carreira</p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 lg:px-8 py-12 lg:py-16">
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar cursos..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-11 rounded-full h-12 bg-card border-border" />
          </div>
          <Button variant="outline" className="rounded-full h-12 px-6 gap-2 border-border" onClick={() => setShowFilters(!showFilters)}>
            ...
            <div className="flex flex-wrap gap-8">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat) => (
                    <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedCategory === cat ? "gradient-bg text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>{cat}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {levels.map((lvl) => (
                    <button key={lvl} onClick={() => setSelectedLevel(selectedLevel === lvl ? null : lvl)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedLevel === lvl ? "gradient-bg text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>{lvl}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {hasFilters && (
          <div className="flex items-center gap-2 mb-6 flex-wrap">
            ...
            {selectedCategory && <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSelectedCategory(null)}>{selectedCategory} <X className="w-3 h-3" /></Badge>}
            {selectedLevel && <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSelectedLevel(null)}>{selectedLevel} <X className="w-3 h-3" /></Badge>}
            {search && <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSearch("")}>"{search}" <X className="w-3 h-3" /></Badge>}
            <button className="text-sm text-accent hover:underline font-sans" onClick={() => { setSelectedCategory(null); setSelectedLevel(null); setSearch(""); }}>Limpar todos</button>
          </div>
        )}
              ...
              return (
                <Link to={`/cursos/${course.slug}`} key={course.id} className="group bg-card rounded-3xl overflow-hidden shadow-card hover:shadow-elevated transition-all duration-500 hover:-translate-y-2">
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-lg font-bold text-foreground mb-2 font-sans group-hover:text-primary transition-colors">{course.title}</h3>
                    <p className="text-sm text-muted-foreground mb-4 line-clamp-2 font-sans">{course.description}</p>
